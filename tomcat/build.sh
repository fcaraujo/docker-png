#!/bin/bash
set -e
TOMCAT_VERSION_TAG=jdk8-openjdk-slim-buster
JMX_AGENT_VERSION=0.13.0
JMX_AGENT_PORT=8686
...
# Add a line
sed -i -e "$line r tomcat/tomcat-jmx-config-string.txt" DockerBuild-$TIME_STRING/catalina.sh
...
# Modify Dockerfile
cp tomcat/Dockerfile DockerBuild-$TIME_STRING
sed -i -e 's#latest#'"$TOMCAT_VERSION_TAG"'#g' DockerBuild-$TIME_STRING/Dockerfile

cd DockerBuild-$TIME_STRING

printf -- '- pattern: '"'"'Catalina<type=Server><>serverInfo: (.+)'"'"'
  name: tomcat_serverinfo
  value: 1
  labels:
    serverInfo: '"'"'$1'"'"'
  type: COUNTER \n' >> tomcat-jmx-exporter.yml
